...
// Soroban's own registration mechanism only ever runs __constructor
// once per contract instance — there is no code path (public entry
// point or otherwise) that can invoke it a second time to test a
// "double initialize" rejection against. The double-initialize
// protection this issue asked for comes from the platform itself, not
// from application logic, so there is nothing to test here beyond what
// the tests above already cover: verify_proof/set_limits/update_vk all
// fail with NotInitialized before __constructor has ever run.
